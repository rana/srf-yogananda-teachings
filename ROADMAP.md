...
| 3a.6 | **Provision Lambda infrastructure + deploy batch functions** | Enable Lambda via `enable_lambda = true` in `dev.tfvars` → `terraform apply` provisions IAM roles, Lambda layer, EventBridge schedules (ADR-017). Deploy book ingestion and chunk relation computation Lambda functions (`/lambda/handlers/`). Deploy database backup Lambda with nightly EventBridge schedule (ADR-019). Replace manual scripts that would exceed Vercel timeouts. CLI wrappers in `/scripts/` for local development. (ADR-016, ADR-017, ADR-019) |
