...
| 3.2 | **Terraform infrastructure + database backup** | `/terraform` directory with modules for Neon (project, database, pgvector), Vercel (project, env vars), Sentry (project, DSN), and backup (S3 bucket + Lambda pg_dump, nightly, 90-day retention). GitHub Actions pipeline calls CI-agnostic `/scripts/` (ADR-070): `terraform plan` on PR, `terraform apply` on merge. State in Terraform Cloud free tier. `.env.example` for local development. Quarterly restore drill: test restore from random backup to Neon branch. (ADR-031, ADR-070, ADR-072) |
...
**Goal:** Introduce audio recordings and photographs as first-class content types, enable multi-media editorial threads, and activate YSS organizational branding.
...
| **Operational staffing** | Editorial review portal provides tooling, but who uses it? Theme tagging, daily passage curation, translation review, and content QA require dedicated staff time. | Establish editorial roles before Phase 5. Key questions for SRF: Who reviews theme tags daily? Who curates daily passages? Who monitors Sentry alerts? Who ingests new books? Who reviews translations? The portal provides the tools; SRF provides the people. |
