...
| 4.15 | **Neptune Analytics setup** | Provision Neptune Analytics graph in the same AWS region as Neon. Load knowledge graph foundation: Teacher, DivineName, Work, Technique, SanskritTerm, Concept, ExperientialState, Place, and Chunk node types from entity_registry, sanskrit_terms, books, and book_chunks tables. Create lineage, authorship, NEXT/PREV sequential chain, and TEACHES edges. Attach Voyage embeddings to Chunk nodes. (ADR-117, DES-054) |
| 4.16 | **Three-path retrieval activation** | Activate Path C (GraphRAG) in the search pipeline: entity recognition in queries via entity_registry → Neptune graph traversal → chunk retrieval → merge with Path A (vector) and Path B (BM25) via RRF. Search pipeline evolves from two-path to three-path. Monitor retrieval quality: Path C must improve relevance on entity-rich queries without degrading keyword queries. (ADR-117, ADR-119, DES-003) |
| 4.17 | **Graph algorithm nightly batch** | Lambda batch job runs PageRank, community detection, and betweenness centrality on the full Neptune graph nightly. Results feed suggestion weights and retrieval confidence scores. Community detection outputs inform theme tagging proposals. (ADR-117, DES-054) |
