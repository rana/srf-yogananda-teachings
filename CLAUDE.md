...
8. **DELTA-compliant analytics.** No user identification, no session tracking, no behavioral profiling. Amplitude event allowlist only. Curation algorithms derive intelligence from the corpus, never from user behavior patterns — even anonymized. (ADR-095, ADR-099)
