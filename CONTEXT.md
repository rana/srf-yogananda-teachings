...
**What's next:** Phase 0a — Prove. Confirm edition + PDF source with SRF, then: repo setup, Neon + schema (pgvector + pg_search + entity_registry), entity registry seed, enrichment prompt design sprint, PDF ingestion with unified enrichment, hybrid search API (vector + BM25 via RRF), search UI, book reader, search quality evaluation (50-query golden set). Twelve deliverables answering one question: does semantic search work over Yogananda's text? Phase 0b (Foundation) adds Vercel deployment, AI librarian enhancements, homepage, and observability. See ROADMAP.md § Phase 0a and § Phase 0b for deliverables. (ADR-113)
...
- [ ] Optimal enrichment prompt structure: test the unified enrichment prompt (ADR-115) against 30 actual passages spanning all document types in the Autobiography before committing the pipeline. Validate consistency of experiential_depth, voice_register, and entity extraction across narrative, poetry, dialogue, and technical instruction.
- [ ] Neptune Analytics vs. Postgres-only graph for bounded corpus: confirm at the Phase 4 gate that Neptune Analytics provides sufficient value over Postgres-based graph queries (recursive CTEs, `ltree`) for a corpus of ~50K chunks. Neptune's value proposition is combined graph traversal + vector similarity in a single query — validate this with actual corpus data before provisioning. (ADR-117)
- [ ] Voyage voyage-3-large vs. voyage-multilingual-2 for CJK-heavy text: when CJK content exists (Phase 10), benchmark both Voyage models against actual Chinese/Japanese/Korean spiritual text. voyage-multilingual-2 may excel on languages with fundamentally different morphology. (ADR-118)
- [ ] fastText vs. alternative language detection for short queries: fastText is strong on longer text but may struggle with 2–3 word spiritual queries containing Sanskrit terms. Evaluate accuracy on spiritual terminology during Phase 0 search quality testing.
- [ ] Multilingual embedding quality evaluation: when multilingual content is available (Phase 10), benchmark Voyage voyage-3-large against alternatives (Cohere embed-v3, BGE-M3) using actual translated passages. Phase 0 English-only evaluation cannot assess cross-language retrieval quality. (ADR-047, ADR-118)
...
- [x] ~~Crisis query detection in search~~ — **Resolved: ADR-122.** Crisis intent added as a category in the search intent classifier. Displays a calm, non-alarmist interstitial above search results with locale-appropriate crisis helpline information. Results are not suppressed. Conservative threshold (false positives acceptable). Crisis resource list requires SRF review. Deliverable 0b.11. (ADR-122, ADR-071, ADR-005 E1)
- [x] ~~Should Presentation mode (currently Phase 7) be pulled to Phase 2 or 3?~~ — **Resolved: Moved to Phase 2 (Deliverable 2.15).** Communal reading is the primary engagement mode in Indian, African, and Latin American cultures. The implementation is a CSS mode — trivial cost. Deferring it 7 phases while individual silent reading was the only mode reproduced a Western cultural hierarchy. (ADR-006, ADR-123)
...
- [ ] Phase 0a is framed as implementing a settled design, but it's actually a hypothesis-testing phase — chunking strategy, enrichment prompts, embedding quality, and search pipeline parameters are all untested guesses. Should Phase 0a success criteria explicitly include a "parameter validation" deliverable that evaluates and adjusts magic numbers (RRF k, chunk size, overlap, debounce, cache TTLs) based on first-contact data? (Relates to ADR-123)
- [ ] AWS Bedrock as sole Claude access path (ADR-014): Bedrock provides consolidated billing and IAM integration, but locks the portal's most variable cost component to a single provider's release cycle and pricing. A thin abstraction in `/lib/services/claude.ts` supporting both Bedrock and direct Anthropic API would cost almost nothing and preserve optionality over the 10-year horizon. Should the abstraction be built from Phase 0? (Relates to ADR-014, ADR-004)
- [ ] Content-addressable passage deep links (ADR-022) use SHA-256 of the first 200 characters. If OCR corrections or edition updates change those characters, all shared links break. Should the resolution chain include a semantic/normalized hash tier (lemmatized, punctuation-stripped) between exact match and fuzzy search? (Relates to ADR-022, ADR-034)
- [ ] Sacred Places fallback hierarchy (DES-023): Google Street View is the sole external visualization fallback for places where SRF virtual tours don't exist, but coverage is unreliable for Indian sites and Google could change terms. Should the schema store a ranked array of external links per place (Street View, commissioned photography, editorial description) rather than assuming one provider? (Relates to DES-023, ADR-070)
...
3. **Single database vs. multi-database:** Two-system architecture: Neon PostgreSQL for content, relational data, embeddings, and primary search; Neptune Analytics for graph intelligence (Phase 4+). Amends the original single-database decision. Phase 0–3 remains single-database; Neptune introduced when graph features become active. *(Resolved by ADR-013, amended by ADR-117.)*
...
5. **Graph intelligence layer:** Neptune Analytics, introduced in Phase 4. Knowledge graph ontology designed from Phase 0 (DES-054). Graph algorithms (PageRank, community detection, betweenness centrality) feed suggestion weights and retrieval confidence. *(Resolved by ADR-117.)*
