...
These are genuine open questions parked until their phase approaches. Triaged from CONTEXT.md during documentation crystallization (2026-02-23). Each question links to the ADR or design section that governs it.

When a phase enters active planning, move its questions back to CONTEXT.md § Open Questions for resolution.
...
### Stakeholder
- [ ] Extreme-context access and institutional intermediaries: lighter-weight surfaces for hospitals, prisons, hospices, refugee camps? Institutional intermediary persona (chaplain, volunteer, hospice worker) accessing on behalf of seekers who cannot browse freely. If SRF affirms institutional access as in-scope, needs design attention by Phase 4. (ADR-006, ADR-071, DES-011, ADR-067)
...
### Stakeholder
- [ ] Does the philanthropist's foundation want to publish "What Is Humanity Seeking?" as a standalone annual communications asset? Communications planning should begin before Phase 6 dashboard ships. (ADR-090)
...
### Technical
- [ ] GitLab migration plan: CI/CD pipeline rewrite (GitHub Actions → GitLab CI), Terraform state backend migration, developer workflow change. Deserves an ADR before Phase 9 planning. (ADR-016, ADR-018, ADR-017)
...
- [ ] Transliteration support for Indic script search suggestions: Romanized input ("samadhi" not "समाधि"), CJK substring matching. (ADR-049, Phase 10)
...
- [ ] Chant-to-recording catalog: does SRF maintain a mapping between chants and recordings? Data import vs. Phase 12 curation task. (ADR-059, ADR-057)
...
## Phase 13–14 (Personalize / Community)

### Stakeholder
- [ ] Does the teaching portal get its own mobile app, or integrate into the SRF/YSS app? (ADR-011)
- [ ] Can SRF provide center location data for "Meditation Near Me" feature? (Phase 14)
