...
Defines the technology stack and infrastructure topology. The frontend runs Next.js on Vercel; the database is Neon PostgreSQL with pgvector and pg_search (single-database, no DynamoDB); AI calls go through AWS Bedrock Claude with model tiering; infrastructure is Terraform-managed from Milestone 1c with GitHub Actions CI/CD, OIDC federation for AWS auth, S3 + DynamoDB state backend, and CI-agnostic deployment scripts. Secrets management uses a two-tier model: AWS Secrets Manager for all credentials (ADR-125), with Vercel OIDC federation eliminating all long-lived AWS credentials (ADR-126). Also covers PWA strategy, content-addressable deep links, rate limiting, PDF generation, low-tech channel support, language URL design, Postgres-native graph intelligence, and three-tier suggestion caching (static JSON → pg_trgm → Vercel KV).
