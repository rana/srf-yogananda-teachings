...
6. Present the draft message to the user and ask for approval or edits before proceeding.
7. Once approved, stage all relevant changed files by name (prefer explicit file names over `git add -A`). Use `git add -A` only if the number of changed files makes individual staging impractical (more than ~15 files).
8. Commit using a HEREDOC for the message to preserve formatting:
   ```
   ...
   ```
9. If the commit fails due to a pre-commit hook, fix the issue, re-stage, and create a NEW commit (never amend).
10. Run `git push` to sync with the remote. If the current branch has no upstream, use `git push -u origin <branch>`.
11. Run `git status` to confirm clean working tree and show the result to the user.
